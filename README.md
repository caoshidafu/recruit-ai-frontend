...
### CandidateCard.vue
候选人卡片组件，采用完整展示设计，默认显示候选人的全部详细信息。

**Props:**
- `candidate`: 候选人对象，包含姓名、经验、学历、技能等信息

**功能:**
- **完整信息展示**: 默认显示候选人的所有详细信息，无需展开操作
- **分区布局**: 头部基本信息、教育经历、推荐理由、关键匹配点、技能标签、工作经历有序排列
- **操作按钮**: 底部提供查看详情、联系候选人、AI分析等操作选项
- **现代化设计**: 圆角卡片、柔和阴影、悬停效果
- **响应式设计**: 移动端、平板、桌面端优化布局

**布局特点:**
- **一页一人**: 每个候选人卡片占据几乎整个视口高度，确保完整展示
- **垂直滚动**: 支持平滑滚动查看下一个候选人
- **匹配度突出**: 右上角绿色匹配度指示器，一目了然
- **信息密度平衡**: 合理的间距和字体大小，保证可读性和信息完整性
