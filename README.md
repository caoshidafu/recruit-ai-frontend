...
- 📊 智能数据大盘，实时展示招聘关键指标和AI智能提醒
- 📈 多维度图表可视化：岗位健康度分析、招聘进度分布、候选人流量趋势
- 🤖 AI智能提醒系统，自动识别招聘异常和优化建议
- ⚡ 优先处理队列，AI推荐最需要关注的岗位和候选人
...
│   │   ├── DashboardView.vue              # 数据大盘主视图组件
│   │   ├── MetricCard.vue                 # 数据指标卡片组件
│   │   ├── BarChart.vue                   # 柱状图组件
│   │   ├── PieChart.vue                   # 饼图组件
│   │   └── LineChart.vue                  # 折线图组件
│   ├── styles/             # 样式目录
...
- ✅ **智能数据大盘功能上线**
  - 新增完整的数据大盘视图，实时展示招聘关键指标和业务状态
  - 实现概览指标卡片：在招岗位、候选人总数、今日面试、待发Offer等核心数据
  - 集成AI智能提醒系统：自动识别招聘异常、简历堆积、面试安排等关键提醒
  - 新增多维度图表可视化：岗位健康度柱状图、招聘进度饼图、候选人流量趋势图
  - 实现优先处理队列：AI推荐最需要关注的岗位，按紧急程度和匹配度排序
  - 完整的mock数据支持：包含8个在招岗位、156位候选人、3个AI智能提醒等真实业务数据
  - 支持数据大盘与岗位详情页面的无缝切换和导航跳转
  - 配置mock管理器使用mock数据，确保数据大盘功能稳定运行
